...
def _estimate_bounds(sdf, s=16, n=32):
    ...
    # else:
    #     raise Exception('Failed to estimate bounds')
...
def generate(
    ...
    # if bounds is None and sdf.aabb.is_finite:
    #     bounds = sdf.aabb
    if bounds is None:
    ...
    if verbose:
        print(
            ...
            f"({num_samples // num_batches:.1f} samples per batch)"
        )
    ...
    #     f = partial(_triangulate, sdf, sparse=sparse)
    #     batches = pool.imap_unordered(f, batches, chunksize=32)
    # else:
    #     batches = map(partial(_triangulate, sdf, sparse=sparse), batches)

    # for result in tqdm(batches, disable=not verbose, desc="Evaluating SDF", total=num_batches):
    #     if result is None:
    #         skipped += 1
    ...
    res = mesh.TriangleMesh().concatenate(submeshes).remove_duplicated_vertices(1e-8)
